...
##' values.  For a directory, outputs individual ascii text files with wavelenght and spectra values for each input
##' ASD file.
##' 
##' @author Shawn P. Serbin
##'
read.asd = function(file.dir=NULL,out.dir=NULL,start.wave=NULL,end.wave=NULL,step.size=NULL,asd.file.ext=".asd",
                    output.file.ext=".csv",spec.dataframe=FALSE,settings.file=NULL){
  ...
  if (!is.null(out.dir)) {
    ...
    out.dir <- paste(settings.file$output.dir,dlm,"ascii_files/",sep="")
  } else {
    ind <- gregexpr(dlm, file.dir)[[1]]
    out.dir <- paste(substr(file.dir,ind[1], ind[length(ind)-1]-1),dlm,"ascii_files",sep="")
  ...
  if (check$isdir){
    ...
    for (i in 1:length(asd.files)){
      tmp <- unlist(strsplit(asd.files[i],paste("\\",asd.file.ext,sep="")))   # <--- remove file extension from file name
...
#         }
      ...
      names(output.spectra) <- c("Wavelength",paste(tmp[1],sep=""))
      output.spectra[,1] <- lambda
      output.spectra[,2] <- out.spec
      out.ascii <- paste(tmp[1],output.file.ext,sep="")     # setup output file name
      
      ### Below for single output directory
      write.csv(output.spectra,paste(out.dir,dlm,out.ascii,sep=""),row.names=FALSE)
    ...
    output <- list(Wavelength=lambda,Spectra=out.spec)  # Spectral data
    return(output)
